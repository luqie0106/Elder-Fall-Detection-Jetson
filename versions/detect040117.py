...
while True:
    ...
    if results.keypoints is not None:
        ...
        for person in filtered_people:
            ...
            if is_fall_event and not (head_feet_close or horizontal_relax_ok):
                is_fall_event = False
            ...
            if lower_body_ok and not sitting_like:
                ...
                # ✅ 改4：持续躺地判断（防漏检）
                if posture == "horizontal" and (head_feet_close or horizontal_relax_ok) and not on_object:
                ...
                if horizontal_relax_ok and not on_object and not sitting_like:
                    state["ground_timer"] += 1
                    if state["ground_timer"] > GROUND_FALL_FRAMES and state["fall_state"] != "FALLEN":
                        state["fall_state"] = "FALLEN"
                        state["fall_timer"] = FALL_HOLD_TIME
