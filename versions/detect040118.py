...
# 仰角场景下更严格确认帧数
UPTILT_FALL_CONFIRM_FRAMES = 5
# 持续躺地触发帧数（调大更紧）
GROUND_FALL_FRAMES = 8
# 新轨迹最少稳定帧（调大更紧）
MIN_TRACK_STABLE_FRAMES = 5
# 仰角场景下最少稳定帧
UPTILT_MIN_TRACK_STABLE_FRAMES = 8
# 判定前至少连续横向帧数（调大更紧）
MIN_HORIZONTAL_FRAMES_FOR_FALL = 1

# 事件去重与重复计数抑制
# 同一位置在短时间内重复触发，视为同一次摔倒
EVENT_DEDUP_FRAMES = 120
EVENT_DEDUP_CENTER_DIST_RATIO = 0.16
EVENT_DEDUP_IOU_TH = 0.25
# 同一 track 在冷却时间内不重复计数
FALL_RECOUNT_COOLDOWN_FRAMES = 240
...
HALF_SQUAT_KNEE_ANGLE_TH = 130.0
HALF_SQUAT_HIP_ANKLE_DY_RATIO_TH = 0.20
HALF_SQUAT_HIP_ANKLE_DX_RATIO_TH = 0.28
...
def detect_fall_sequence(history):
    ...
    # 主路径：完整证据
    strong_path = cond1 and cond2 and cond2b and cond3 and cond4
    # 恢复兜底：当髋部跟踪抖动时允许触发（仍要求姿态变化+下落+速度+冲击）
    fallback_path = cond1 and cond2 and cond3 and cond4
...
while True:
    ...
    if results.keypoints is not None:
        ...
        for person in filtered_people:
            ...
            standing_support_like = is_standing_bending(keypoints, dyn_bend_hip_th, dyn_bend_shoulder_th) and not head_feet_close
            ...
            non_fall_posture_like = sitting_like or half_squat_like or kneeling_like or standing_support_like
            ...
            if (uptilt_like or high_uptilt_mode) and is_fall_event and not (head_feet_close or horizontal_relax_ok):
                is_fall_event = False

            color = (0, 255, 0)

            # 2️⃣ 状态机（每个人独立）
            required_confirm_frames = UPTILT_FALL_CONFIRM_FRAMES if uptilt_like else FALL_CONFIRM_FRAMES
            required_ground_frames = HIGH_UPTILT_GROUND_FRAMES if high_uptilt_mode else GROUND_FALL_FRAMES
            min_stable_frames = UPTILT_MIN_TRACK_STABLE_FRAMES if uptilt_like else MIN_TRACK_STABLE_FRAMES
            ...
            if lower_body_ok and not non_fall_posture_like:
                ...
                # ✅ 改4：持续躺地判断（防漏检）
                if posture == "horizontal" and state["horizontal_frames"] >= MIN_HORIZONTAL_FRAMES_FOR_FALL and (head_feet_close or horizontal_relax_ok) and not on_object:
            ...
            if state["fall_state"] == "FALLEN":
                ...
                if posture == "horizontal" and (head_feet_close or horizontal_relax_ok) and lower_body_ok and not sitting_like and not on_object:
                    state["fall_timer"] = FALL_HOLD_TIME
